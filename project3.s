...
reviewString: #function compares to check if string input from user is valid and within the restrictions
	...
	bne $t6, $zero, input_IsEmpty
	slti $t6, $t5, 117               # conditional: input < ascii(117) aka u input is valid
	bne $t6, $zero,  moveCharForward
	bgt $t5, 118, input_IsEmpty   # conditional: input > ascii(118) aka v is input invalid
 #Spacing:
...
#addi $sp, $sp, -4 #alocating space of 4 bytes into the stack pointer
 ...
 #following calculates conversions to base N (31)number 
 upperBase31:
 	  addi $s4, $s4, -55
  	j serialize
 lowerBase31:
...
print:
	...
	#conversion_subprogram: 
	 	# conversion functions need to go here 
		#prints 
		#li $v0, 1
		#move $a0, $s0
		#syscall
